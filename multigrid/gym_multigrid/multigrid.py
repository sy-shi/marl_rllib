...
class WorldObj:
    ...
    def encode(self, world, current_agent=False):
        ...
        if world.encode_dim == 0:
            return (world.OBJECT_TO_IDX[self.type], 0, 0, world.COLOR_TO_IDX[self.color])
        elif world.encode_dim == 3:
            return (world.OBJECT_TO_IDX[self.type], world.COLOR_TO_IDX[self.color], 0)
            # return (world.OBJECT_TO_IDX[self.type],0,0)
        elif world.encode_dim == 2:
            return (world.OBJECT_TO_IDX[self.type], 0)
        else:
...
class Door(WorldObj):
    ...
    def encode(self, world, current_agent=False):
        ...
        if self.is_open:
            ...
            state = 1
        if world.encode_dim == 0:
            return (world.OBJECT_TO_IDX[self.type], world.COLOR_TO_IDX[self.color], state, 0)
        return (world.OBJECT_TO_IDX[self.type], state)
...
class Key(WorldObj):
    def __init__(self, world, color='blue', reward=1):
...
class Ball(WorldObj):
    def __init__(self, world, index=0, reward=1):
        ...
        self.index = index
        self.reward = reward
        # self.required_remove = 2
        # self.times_to_remove = self.required_remove
        # self.remove_step = -1
        # self.i = None
    ...
    # def remove(self, remove_step, i):
    #     # print("in heal, step count:", heal_step)
    #     # print("healing agent:", i)
    #     if remove_step - self.remove_step == 1:
    #         if self.i == None or self.i == i:
    #             self.i = i
    #             self.times_to_remove -= 1            
    #     else:
    #         self.i = i
    #         self.times_to_remove = self.required_remove - 1
    #     self.remove_step = remove_step
    #     if self.times_to_remove == 0:
    #         return True
    #     else:
    #         return False
    ...
    #         return (world.OBJECT_TO_IDX[self.type], world.COLOR_TO_IDX[self.color], 0, 1, 0, 0)
    #     if world.encode_dim == 0:
    #         return (world.OBJECT_TO_IDX[self.type], 0, self.times_to_remove, world.COLOR_TO_IDX[self.color])
    #     return super().encode(world, current_agent)
...
class Victim(WorldObj):
    def __init__(self, world, index, reward=1, color=None):
        if color is None:
            super().__init__(world, 'victim', world.IDX_TO_COLOR[index])
        else:
            super().__init__(world, 'victim', world.IDX_TO_COLOR[color])
        self.index = index
        self.reward = reward
        self.if_healed = False
        self.required_heal = 3
        self.times_to_heal = self.required_heal
        self.heal_step = -1
        self.i = None
    ...
    def heal(self, heal_step, i):
        # print("in heal, step count:", heal_step)
        # print("healing agent:", i)
        if heal_step - self.heal_step == 1:
            if self.i == None or self.i == i:
                self.i = i
                self.times_to_heal -= 1            
        else:
            self.i = i
            self.times_to_heal = self.required_heal - 1
        self.heal_step = heal_step
        if self.times_to_heal == 0:
            return True
        else:
            return False
    ...
    def encode(self, world, current_agent=False):
        if world.encode_dim > 3:
            return (world.OBJECT_TO_IDX[self.type], world.COLOR_TO_IDX[self.color], 0, 1, 0, 0)
        if world.encode_dim == 0:
            return (world.OBJECT_TO_IDX[self.type], 0, self.times_to_heal, world.COLOR_TO_IDX[self.color])
        return super().encode(world, current_agent)

    def render(self, img):
        c = COLORS[self.color]
        fill_coords(img, point_in_rect(0.4,0.6,0.2,0.8), c)
        fill_coords(img, point_in_rect(0.2,0.8,0.4,0.6), c)

class Healed_Victim(WorldObj):
    def __init__(self, world, index, color='green'):
        super().__init__(world, 'healed_victim', color)
        self.index = index
...
class Agent(WorldObj):
    ...
    def encode(self, world, current_agent=False):
        ...
        if world.encode_dim == 0:
            if current_agent:
                return (world.OBJECT_TO_IDX[self.type], self.dir, 1, 0)
            else:
                return (world.OBJECT_TO_IDX[self.type], self.dir, 0, 0)
        elif world.encode_dim == 2:
            if current_agent:
                return (world.OBJECT_TO_IDX[self.type], self.dir)
            else:
                return (world.OBJECT_TO_IDX['other_agent'], self.dir)
        
        elif world.encode_dim == 3:
            return (world.OBJECT_TO_IDX[self.type], world.COLOR_TO_IDX[self.color], self.dir)
            # if current_agent:
            #     return (world.OBJECT_TO_IDX[self.type], 1, self.dir)
            # else:
            #     return (world.OBJECT_TO_IDX[self.type], 0, self.dir)
        elif self.carrying:
            if current_agent:
...
class Grid:
    ...
    def encode(self, world, vis_mask=None):
        ...
        array = np.zeros((self.width, self.height, (4 if world.encode_dim==0 else world.encode_dim)), dtype='uint8')
...
class Actions:
    available=['still', 'left', 'right', 'forward', 'heal', 'pickup', 'drop', 'toggle']
...
class USARActions:
    # available=['still', 'left', 'right', 'forward', 'pickup', 'drop', 'heal', 'done']
    available = ['still', 'left', 'right', 'forward', 'heal']
...
class MultiGridEnv(gym.Env):
    ...
    def __init__(
        ...
        if partial_obs:
            self.observation_space = spaces.Box(
                low=0,
                high=255,
                shape=(agent_view_size, agent_view_size, (4 if self.objects.encode_dim==0 else self.objects.encode_dim)),
                dtype='uint8'
            )
            ...
            self.observation_space = spaces.Box(
                low=0,
                high=255,
                shape=(width, height, (4 if self.objects.encode_dim==0 else self.objects.encode_dim)),
                dtype='uint8'
            )
    ...
    def reset(self):
        ...
        # obs=[{"image": self.objects.normalize_obs*ob} for ob in obs]
        return obs
    ...
    def step(self, actions):
        ...
        for i in order:
            ...
            fwd_pos_s.append(fwd_pos)
            # Get the contents of the cell in front of the agent
            fwd_cell = self.grid.get(*fwd_pos)
            fwd_cell_s.append(fwd_cell)
            ...
            # Move forward
                ...
                if fwd_cell is None or fwd_cell.can_overlap(): 
                    ...
                    self.grid.set(*self.agents[i].pos, Floor(self.world))
            ...
            if len(self.actions.available) == 8:
                # heal victim individually
                if actions[i] == self.actions.heal:
                    dones[i] = self._handle_heal(i, rewards, fwd_pos, fwd_cell)
                # Pick up key
                elif actions[i] == self.actions.pickup:
                    self._handle_pickup(i, rewards, fwd_pos, fwd_cell)
                # Remove rubble
                elif actions[i] == self.actions.remove:
                    # print("handle remove")
                    self._handle_remove(i, rewards, fwd_pos, fwd_cell)
                # Toggle/activate an object
                elif actions[i] == self.actions.toggle:
                    self._handle_toggle(i, rewards, fwd_pos, fwd_cell)
        ...
        # handle healing action of USAR
        # done, healed = self._handle_heal(rewards, fwd_pos_s, fwd_cell_s, actions)
        # if len(fwd_cell_s)>0 and fwd_cell_s[0].can_heal() and fwd_cell_s[0].if_healed:
        #     self.grid.set(*fwd_pos_s[0], Healed_Victim(self.world, fwd_cell_s[0].index))
        for d in dones:
            if d is True:
                done = True
    ...
    def render(self, mode='human', close=False, highlight=False, tile_size=TILE_PIXELS):
        ...
        #     self.agents[i].render(img)
        if mode == 'human':
            self.window.show_img(img)
        World.encode_dim = World.encode_dim_init
        return img

class MultiRoomGrid(MultiGridEnv):
    def __init__(self, config, start_rooms, goal_rooms, rubble_rooms, room_size=3, victim_reward = 1, rubble_reward=1,
                agents_index = [],
                view_size=7,
                max_steps = 10000,
                action_set = Actions,
        ...
        # Initialize static properties of the environment
        # ATTENTION: all environment variables should be put into reset()
        self.num_rows = len(config)
        self.num_cols = len(config[0])
        self.room_size = room_size
        self.start_rooms = start_rooms
        self.goal_rooms = goal_rooms
        self.rubble_rooms = rubble_rooms
        self.config = config
        self.max_tries = 100
        self.line_rooms = None

        self.world_width = 13 # Sum of room sizes + 1 space extra for walls.
        self.world_height = 13 # Sum of room sizes + 1 space extra for walls.
        ...
        super().__init__(
            grid_size=None,  # length of the whole grid world
            width=self.world_width, # if grid_size = 0, of the whole grid world
            height=self.world_height, # of the whole grid world
            max_steps= max_steps,
    ...
    def _sample_room(self, ul):
        ...
        while try_idx < self.max_tries:
            loc = (np.random.randint(low=ul[0]+1, high=ul[0]+(self.room_size + 1)), 
                   np.random.randint(low=ul[1]+1, high=ul[1]+(self.room_size + 1)))
            try_idx += 1
            try_status = True
            if self.grid.get(*loc) == None or type(self.grid.get(*loc)) == Floor:
                # return loc
                for ai in range(len(self.agents)):
                    if (self.agents[ai].pos is not None and np.allclose(loc, self.agents[ai].pos)):
                        try_status = False
                        break
                if not try_status:
                    continue
    ...
    def _gen_grid(self, width, height, agent=None, victim_pos=None):
        # Create an empty grid
        self.grid = Grid(width, height)

        self.mission = ""
        
        # Create the surronding walls
        for i in range(0, width):
            self.grid.set(i, 0, Wall(self.world))
            self.grid.set(i, height - 1, Wall(self.world))
        for j in range(0, height):
            self.grid.set(0, j, Wall(self.world))
            self.grid.set(width - 1, j, Wall(self.world))
        ...
        # Create empty hallways
        for (i,j) in [(4,2), (8,2), (10,4), (4,10), (8,10), (2,8)]:
            self.grid.set(i,j,None) 


        # Sample 2 victim pos
        non_block_rooms = [(2,2), (10,10)]
        blocked_rooms = [(2,6), (10,6)]
        block_rooms = [(2,10), (10,2)]
        room_idxs = np.random.randint(low=0, high=2, size=2)
        self.victim_pos = [None, None]
        # non-blocked victim pos
        self.victim_pos[0] = self.place_obj(Victim(self.world, index=2, reward=self.victim_reward), top=non_block_rooms[1-room_idxs[1]], size=(1,1))
        self.victim_pos[1] = self.place_obj(Victim(self.world, index=2, reward=self.victim_reward), top=blocked_rooms[room_idxs[1]], size=(1,1))
        
        # self.victim_pos[0] = self.place_obj(Victim(self.world, index=2, reward=self.victim_reward), top=(non_block_rooms[1-room_idxs[1]][0]-1,non_block_rooms[1-room_idxs[1]][1]-1) , size=(3,3))
        # self.victim_pos[1] = self.place_obj(Victim(self.world, index=2, reward=self.victim_reward), top=(blocked_rooms[room_idxs[1]][0]-1,blocked_rooms[room_idxs[1]][1]-1) , size=(3,3))
        
        # key pos
        # self.key_pos = self.place_obj(Key(self.world, color = "green"), top = (non_block_rooms[room_idxs[1]][0]-1, non_block_rooms[room_idxs[1]][1]-1), size = (3,3))
        self.key_pos = self.place_obj(Key(self.world, color = "green"), top = (non_block_rooms[room_idxs[1]][0], non_block_rooms[room_idxs[1]][1]), size = (1,1))

        # 2 door pos
        door_pos = (4,2) if (1-room_idxs[1] == 0) else (8,10)
        self.grid.set(*door_pos, Door(self.world, "green", is_open=False, is_locked=True))
        # door_pos = (8,2) if door_pos == (8,10) else (4,10)
        # self.grid.set(*door_pos, Door(self.world, "green", is_open=False, is_locked=True))

        # Place 4 rubbles:
        self.rubble_pos = [None, None, None]
        # rubble blocks the agent, should be moved first
        poss = []
        if room_idxs[1] == 0:
            poss.append((3,10))
            poss.append((2,9))
            ...
            poss.append((10,3))
        for i in range(2):
            self.rubble_pos[i] = self.place_obj(Ball(self.world, index=3, reward = 0.5), top=poss[i], size=(1,1))
        # rubble that shouldn't block victim.
        free_rooms = [non_block_rooms[room_idxs[1]], blocked_rooms[1-room_idxs[1]], block_rooms[1-room_idxs[1]]]
        free_rooms = [blocked_rooms[1-room_idxs[1]], block_rooms[1-room_idxs[1]]]
        free_rooms = [non_block_rooms[room_idxs[1]], block_rooms[1-room_idxs[1]]]
        room_idx = np.random.randint(low=0, high=2)
        self.rubble_pos[2] = self.place_obj(Ball(self.world, index=3, reward = 0.5), top=free_rooms[1], size=(1,1))

        # Sample agent start location
        for ai in range(len(self.agents)):
            self.agents[ai].pos = self.place_agent(self.agents[ai],top = (5,5), size = (3,3))
    ...
    def reset(self, agent=None, victim_pos = None):
        ...
        # To keep the same grid for each episode, call env.seed() with
        # the same seed before calling env.reset()
        self._gen_grid(self.width, self.height, agent, victim_pos)
